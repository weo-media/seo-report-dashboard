...
// for later when there is a form to fill out custom input
// "9": {
//   icon: "default", [["all" or tally box identifier, svg icon string or allIcons.icon], ...],
//   title: "default", [["all" or tally box identifier, string], ...],
//   moreInfo: "default", [["all" or tally box identifier, string], ...],
//   toolTip: "default", [["all" or tally box identifier, string], ...],
//   count: "default", [["all" or tally box identifier, number or string], ...],
//   countSubText: "default", [["all" or tally box identifier, string], ...],
//   movement: "default", [["all" or tally box identifier, "up" or "down" or null], ...],
// }
const customInput = {
  "9": {
...
function prepData(typeNum, customInput) {
  ...
  if (typeNum === "9") {
    ...
    const curMonthDate = data[data.length - 1][0];
    ...
    function startWithThisMonth(normalOrReversed) {
      ...
      startMonth.forEach((x, i) => {
        ...
        if (tlyBxData.count === 'default') {
          ...
          tlyBxData.count = ifNotCustomInputUseDefault(tlyBxData.count, 'err');
        }
        ...
        // set title
        tlyBxData.title = ifNotCustomInputUseDefault(tlyBxData.title, x[1]);
        ...
        // set moreinfo 
        tlyBxData.moreInfo = ifNotCustomInputUseDefault(tlyBxData.moreInfo, 'Reports indicate we received ' + tlyBxData.count + ' form submissions of the \"' + tlyBxData.title + '\" form originating from your site durning the last month.');
        ...
        function ifNotCustomInputUseDefault(tlyBxOpt, defaultOption) {
          return tlyBxOpt && tlyBxOpt !== 'default' && tlyBxOpt.some((item) => item[0] === x[2]) ? tlyBxOpt.filter((item) => item[0] === x[2])[0][1] : defaultOption;
        };
...
function buildTallyBox(tlyBx) {
  ...
  btn.id = 'tallyEmailForm-' + tlyBx.dataSet;
  ...
  btn.innerHTML = '<div class=\"tally-main\" ' + (() => {
    if (tlyBx.movement === 'up') { return "style=\"background: linear-gradient(90deg, #70B44E 0%, #89DB60 73.27%);\""; }
    if (tlyBx.movement === 'down') { return "style=\"background: linear-gradient(90deg, #D94F31 0%, #F07154 73.27%);\""; }
